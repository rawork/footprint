...
class PageManager extends ModelManager {
	...
	public function getUrl($node) {
		return trim($node['url']) ?: ($this->get('session')->get('locale') != PRJ_LOCALE ? '/'.$this->get('session')->get('locale') : '').$this->get('routing')->getGenerator()->generate('public_page', array('node' => $node['name']));
	}
