...
class SelectTreeType extends LookUpType {
	...
	protected function select_tree_getInput($value, $name) {
		...
		$defaultValue = $value ? '  <input type="radio" name="'.$input_id.'_default" value="'.$value.'" class="selected-default" data-input-id="'.$input_id.'" checked> Default <a href="#" class="selected-remove" data-input-id="'.$input_id.'"><i class="glyphicon glyphicon-remove"></i></a>' : '';
		$ret = '
<div id="'.$input_id.'_title">
<div>'.$staticValue.$defaultValue.'</div> 
'.$extraElements.'	
</div>
<button class="btn btn-success" href="javascript:void(0)" type="button" onClick="showTreeDialog(\''.$input_id.'\',\''.$table.'\',\''.$name.'\', \''.$id.'\', \''.htmlspecialchars($this->getStatic($value)).'\',\''.$value.'\');">Select</button>
