	...
	private function getSelectInput($value, $name) {
		...
		if ('many' == $this->getParam('link_type')  && $this->dbId) {
			...
			foreach ($entities as $entity) {
				$extraElements[] = '<div>'.$this->getStatic($entity['id']).' <input type="radio" name="'.$input_id.'_default" value="'.$entity['id'].'" class="selected-default" data-input-id="'.$input_id.'">  Default <a href="#" class="selected-remove" data-input-id="'.$input_id.'"><i class="glyphicon glyphicon-remove"></i></a></div>';
				$extra[] = $entity['id'];
			}
		}
		$extra = implode(',', $extra);
		...
		$defaultValue = $value ? ('many' == $this->getParam('link_type') ? '  <input type="radio" name="'.$input_id.'_default" value="'.$value.'" class="selected-default" data-input-id="'.$input_id.'" checked> Default' : '').' <a href="#" class="selected-remove" data-input-id="'.$input_id.'"><i class="glyphicon glyphicon-remove"></i></a>' : '';
		$ret = '
<div id="'.$input_id.'_title">
<div>'.$staticValue.$defaultValue.'</div>
'.$extraElements.'
</div>
<button class="btn btn-success" href="javascript:void(0)" type="button" onClick="showSelectDialog(\''.$input_id.'\',\''.$table.'\',\''.$name.'\', \''.$id.'\', \''.htmlspecialchars($this->getStatic($value)).'\',\''.$value.'\');">Select</button>
