...
class TemplateType extends Type {
	...
	public function getBackupPath($filename) {
		return PRJ_REF.TWIG_BACKUP_PATH.$filename;
	}
	
	public function getBackupRealPath($filename) {
		return PRJ_DIR.TWIG_BACKUP_PATH.$filename;
	}
	...
	public function getInput($value = '', $name = '') {
		...
		if ($content = $this->getStatic()) {
			$content = '<div id="'.$name.'_delete">Current version: '.$content.'<label for="'.$name.'_delete"><input name="'.$name.'_delete" type="checkbox" id="'.$name.'_delete"> delete</label></div>';
			...
			if (count($versions)) {
				$content .= '<select class="form-control" onChange="changeTemplateState(\''.$name.'\')" id="'.$name.'_version" name="'.$name.'_version"><option value="0">View archive</option>'."\n";
				foreach ($versions as $version) {
					$content .= '<option value="'.$version['id'].'">'.$version['created'].'</option>';
				}
				$content .= '</select> <div class="view-button hidden" id="'.$name.'_view"><input type="button" class="btn btn-success" onClick="showTemplateVersion(\''.$name.'\')" value="View"></div>';
			}
		}
		if (empty($value)){
			$text = '
<input type="hidden" name="'.$name.'_oldvalue" value="'.$this->dbValue.'">
<label for="'.$name.'_cre"><input name="'.$name.'_cre" type="checkbox" id="'.$name.'_cre" onClick="changeTemplateWidget(\''.$name.'\')"> Create</label>
<div class="hidden" id="'.$name.'_template">
<input class="form-control" type="text" name="'.$name.'" placeholder="File name">
<textarea class="form-control" wrap="off" name="'.$name.'_template" placeholder="Template" rows="15"></textarea>
</div>
